...
from services.calendar.outlook_sync import (
    exchange_code_for_tokens_outlook,
    get_outlook_auth_url,
    _get_primary_calendar_id_outlook,
)
...
@router.get("/integrations/outlook/auth-url", response_model=GoogleAuthUrlResponse)
async def outlook_auth_url(
    ...
    if not settings.MICROSOFT_CLIENT_ID:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Outlook integratie is niet geconfigureerd op deze server.",
        )
    try:
        auth_url = get_outlook_auth_url(redirect_uri=redirect_uri, state=str(member.id))
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(e))
    return GoogleAuthUrlResponse(auth_url=auth_url)
...
async def connect_outlook_calendar(
    request: Request,
    payload: OutlookCallbackRequest,
    member: Member = Depends(get_current_member),
    db: AsyncSession = Depends(get_db),
):
    """Exchange the Microsoft OAuth code for tokens and store the Outlook integration."""
    try:
        token_data = await exchange_code_for_tokens_outlook(payload.code, payload.redirect_uri)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    ...
    if not access_token:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Geen access token ontvangen van Microsoft.")

    try:
        calendar_id = await _get_primary_calendar_id_outlook(access_token)
    except Exception:
        calendar_id = "primary"
...
async def get_event_linked_tasks(
    ...
    """Return all tasks linked to a calendar event (generated by AI)."""
    from models.task import Task
    from schemas.task import TaskResponse

    result = await db.execute(
        select(Task).where(
            Task.linked_calendar_event_id == event_id,
...
async def sync_calendars(
    ...
    Rate limited to 10 per hour to avoid hammering the Google/Microsoft API.
    """
    if member.role == MemberRole.caregiver:
